...
#--------------------------------------------------------------------------------------------------#
###Define sequential average Hub


getHub_of_dailyGraphs <- function(edge_list, node_list, 
                                  param_u,param_d){
  ...
  ##
  ##Calculate Hub for a list of 365 networks
  ##
  ...
  for(i in 1:365){
    FC_edge_list_3[[i]]<-FC_edge_list_2%>%
    ...
    list_Hub_FC[[i]]<- igraph::hub_score(graphs_FC[[i]])$vector%>%
      as.data.frame()
    
    
    list_Hub_FC[[i]]$names<-row.names(list_Hub_FC[[i]])
    colnames(list_Hub_FC[[i]])[1]<-"Hub"
    
    list_Hub_FC[[i]]<- list_Hub_FC[[i]]%>% 
      summarise(mean_Hub= mean(Hub))
    
    list_Hub_FC[[i]]$threshold<- i
  }
  
  
  return(list(list_Hub_FC))
...
#--------------------------------------------------------------------------------------------------#
###calculate average Hub score for a list of 365 networks
...
comp1<-getHub_of_dailyGraphs(edge_list=edgeList_before, 
                             node_list=nodes_before,
                             param_u,param_d)

comp1_FC<-data.frame(matrix(unlist(comp1[[1]]), nrow=length(comp1[[1]]), byrow=TRUE)) %>%
  mutate(mean_Hub= X1, threshold= X2, Period= "Before")%>% select(mean_Hub, threshold, Period)%>%
  group_by(threshold)

###short-term after,
comp2<-getHub_of_dailyGraphs(edge_list=edgeList_shortT_after, 
                             node_list=nodes_shortT_after,
                             param_u,param_d)

comp2_FC<-data.frame(matrix(unlist(comp2[[1]]), nrow=length(comp2[[1]]), byrow=TRUE)) %>%
  mutate(mean_Hub= X1, threshold= X2, Period= "Short-term after")%>%select(mean_Hub, threshold, Period)%>%
  group_by(threshold)


###Long-term after,
comp3<-getHub_of_dailyGraphs(edge_list=edgeList_longT_after, 
                             node_list=nodes_longT_after,
                             param_u,param_d)

comp3_FC<-data.frame(matrix(unlist(comp3[[1]]), nrow=length(comp3[[1]]), byrow=TRUE)) %>%
  mutate(mean_Hub= X1, threshold= X2, Period= "Long-term after")%>%select(mean_Hub, threshold, Period)%>%
  group_by(threshold)
...
#--------------------------------------------------------------------------------------------------#
###Plot average Hub score for a list of 365 networks


###Plots
p4<-ggplot()+
  geom_step(data=comp1_FC, mapping=aes(x = threshold, y= mean_Hub, color= Period),
            direction = "vh",lwd = 1.2 )+
  geom_step(data=comp2_FC, mapping=aes(x = threshold, y= mean_Hub,color=Period),
            direction = "vh",lwd = 1.2   )+
  geom_step(data=comp3_FC, mapping=aes(x = threshold, y= mean_Hub,color=Period),
            direction = "vh",lwd = 1.2  )+
  xlab("Minimum days of LHC") +
  ylab("Mean Hub Centrality") +
  ...
  theme_bw()+
  theme(legend.position ="right",axis.text.x = element_text(size = 12, angle = 45, hjust=1)) +
  scale_color_manual(values= c("#EBCC2A", "#78B7C5", "#3B9AB2"))+
  ggtitle(expression("Average Hub Centrality daily LHC thresholds")) 

p4

png(file = "hub_365thresholds.png", res = 300, width = 2000, height = 1315)
plot (p4)
dev.off()
