...
#--------------------------------------------------------------------------------------------------#
###Define sequential average weidghted degree/strength

getStrength_of_dailyGraphs <- function(edge_list, node_list, param_u,param_d){
  ...
  FC_edge_list_2<-FC_edge_list %>%
    ...
    mutate(hyd_up_val2_c=LHC.y) %>%
    mutate(hc=pmin(hyd_up_val1_c,hyd_up_val2_c))%>% #min values of hc btw. each pair of nodes
    select(from,to,value,weight,hc)
  
  
  
  ##
  ##Calculate weighted degree for a list of 365 networks
  ##
  
  ##with cost/resistance as link attribute
  
  list_strength_SC <- list()
  SC_edge_list_3<-list()
  ...
  for(i in 1:365){
    ...
    list_strength_SC[[i]]<- igraph::graph_from_data_frame(SC_edge_list_3[[i]],
                                                      directed = TRUE,
                                                      vertices = graph_df_bidir_v)%>%
      strength(mode = c("all"))%>%
      as.data.frame()
    
    list_strength_SC[[i]]$names<-row.names(list_strength_SC[[i]])
    colnames(list_strength_SC[[i]])[1]<-"strength"
    
    list_strength_SC[[i]]<- list_strength_SC[[i]]%>% 
      summarise(mean_strength= mean(strength))
    
    list_strength_SC[[i]]$threshold<- i
  }
  
  ##with dispersal probabilities as link attribute
  
  FC_edge_list_3<-list()
  list_strength_FC <- list()
  graphs_FC<-list()
  
  for(i in 1:365){
  
    FC_edge_list_3[[i]]<-FC_edge_list_2%>% filter(hc > i)%>%select(-c(hc))
  
  
    graphs_FC[[i]]<- igraph::graph_from_data_frame(FC_edge_list_3[[i]],
                                                 directed = TRUE,
                                                 vertices = graph_df_bidir_v)
    list_strength_FC[[i]]<-igraph::strength(graphs_FC[[i]],
                                            mode = c("all"))%>% as.data.frame()
    
  
    list_strength_FC[[i]]$names<-row.names(list_strength_FC[[i]])
  
    colnames(list_strength_FC[[i]])[1]<-"strength"
  
  
    list_strength_FC[[i]]<- list_strength_FC[[i]]%>% 
      dplyr::summarise(mean_strength= mean(strength))
  
    list_strength_FC[[i]]$threshold<- i
  }

  return(list(list_strength_SC,list_strength_FC))
...
#--------------------------------------------------------------------------------------------------#
###calculate average weighted degree/strength for a list of 365 networks
...
###Before
comp1<-getStrength_of_dailyGraphs(edge_list=edgeList_before, 
                                  node_list=nodes_before,
                                  param_u,param_d)

#For cost/resistance attribute

comp1_SC<-data.frame(matrix(unlist(comp1[[1]]), nrow=length(comp1[[1]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Before")%>% select(mean_strength, threshold, Period)%>%
  group_by(threshold)

#For dispersal probability attribute

comp1_FC<-data.frame(matrix(unlist(comp1[[2]]), nrow=length(comp1[[2]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Before")%>% select(mean_strength, threshold, Period)%>%
  group_by(threshold)


###short-term after,
comp2<-getStrength_of_dailyGraphs(edge_list=edgeList_shortT_after, 
...
comp2_SC<-data.frame(matrix(unlist(comp2[[1]]), nrow=length(comp2[[1]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Short-term after")%>%select(mean_strength, threshold, Period)%>%
  group_by(threshold)

#For  dispersal probability attribute
comp2_FC<-data.frame(matrix(unlist(comp2[[2]]), nrow=length(comp2[[2]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Short-term after")%>%select(mean_strength, threshold, Period)%>%
  group_by(threshold)


###Long-term after,
comp3<-getStrength_of_dailyGraphs(edge_list=edgeList_longT_after, 
                                  node_list=nodes_longT_after,
                                  param_u,param_d)

#For cost/resistance attribute
comp3_SC<-data.frame(matrix(unlist(comp3[[1]]), nrow=length(comp3[[1]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Long-term after")%>%select(mean_strength, threshold, Period)%>%
  group_by(threshold)

#For  dispersal probability attribute
comp3_FC<-data.frame(matrix(unlist(comp3[[2]]), nrow=length(comp3[[2]]), byrow=TRUE)) %>%
  mutate(mean_strength= X1, threshold= X2, Period= "Long-term after")%>%select(mean_strength, threshold, Period)%>%
  group_by(threshold)
...
#--------------------------------------------------------------------------------------------------#
###Plot average weighted degree/strength for a list of 365 networks

#For cost/resistance attribute
p<-ggplot()+
  geom_step(data=comp1_SC, mapping=aes(x = threshold, y= mean_strength, color= Period),
            direction = "vh",lwd = 1.2 )+
  geom_step(data=comp2_SC, mapping=aes(x = threshold, y= mean_strength,color=Period),
            direction = "vh",lwd = 1.2   )+
  geom_step(data=comp3_SC, mapping=aes(x = threshold, y= mean_strength,color=Period),
            direction = "vh",lwd = 1.2  )+
  ...
  ggtitle(expression("Average WD (based on cost) at daily LHC thresholds")) 

p

png(file = "strength_365thresholds_SC_netdist.png", res = 300, width = 2000, height = 1315)
plot (p)
dev.off()
...
#For  dispersal probability attribute
p2<-ggplot()+
  geom_step(data=comp1_FC, mapping=aes(x = threshold, y= mean_strength, color= Period),
            direction = "vh",lwd = 1.2 )+
  geom_step(data=comp2_FC, mapping=aes(x = threshold, y= mean_strength,color=Period),
            direction = "vh",lwd = 1.2   )+
  geom_step(data=comp3_FC, mapping=aes(x = threshold, y= mean_strength,color=Period),
            direction = "vh",lwd = 1.2  )+
  xlab("Minimum days of LHC") +
  ylab("Mean strength") +
  #ggtitle("Normalized dPC index for different floodplain regions")+
  ...
  theme(legend.position ="top",axis.text.x = element_text(size = 12, angle = 45, hjust=1)) +
  scale_color_manual(values= c("#EBCC2A", "#78B7C5", "#3B9AB2"))+
  ggtitle(expression("Average WD (based on disp. prob.) at daily LHC thresholds")) 

p2

png(file = "strength_365thresholds_FC_costdist.png", res = 300, width = 2000, height = 1315)
plot (p2)
dev.off()
...
p3<-ggarrange(
  p,   p2, ncol = 1, labels = c("A", "B")) 
p3
png(file = "strength_365thresholds_SC_FC.png", res = 300, width = 2000, height = 1694)
plot (p3)
dev.off()
