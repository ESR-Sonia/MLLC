...
#--------------------------------------------------------------------------------------------------#
###Define sequential average PageRank

getPR_of_dailyGraphs <- function(edge_list, node_list, param_u,
                                 param_d){
  ...
  ##
  ##Calculate PageRank for a list of 365 networks
  ##
  ...
  for(i in 1:365){
    ...
    SC_graph_list[[i]]<- igraph::graph_from_data_frame(SC_edge_list_3[[i]],
                                                         directed = TRUE,
                                                         vertices = graph_df_bidir_v)
    list_multiPR_SC[[i]]<- igraph::page_rank(SC_graph_list[[i]])$vector%>%
      as.data.frame()
    
    
    list_multiPR_SC[[i]]$names<-row.names(list_multiPR_SC[[i]])
    colnames(list_multiPR_SC[[i]])[1]<-"PageRank"
    
    list_multiPR_SC[[i]]<- list_multiPR_SC[[i]]%>% 
      summarise(max_PageRank= max(PageRank))
    
    list_multiPR_SC[[i]]$threshold<- i
    
    
    }
  
  return(list(list_multiPR_SC))
...
#--------------------------------------------------------------------------------------------------#
###calculate average PageRank for a list of 365 networks
...
############
###Before
comp1<-getPR_of_dailyGraphs(edge_list=edgeList_before, 
                            node_list=nodes_before,
                            param_u,param_d)


comp1_SC<-data.frame(matrix(unlist(comp1[[1]]), nrow=length(comp1[[1]]), byrow=TRUE)) %>%
  mutate(max_PageRank= X1, threshold= X2, Period= "Before")%>% select(max_PageRank, threshold, Period)%>%
  group_by(threshold)


###short-term after,

comp2<-getPR_of_dailyGraphs(edge_list=edgeList_shortT_after, 
                            node_list=nodes_shortT_after,
                            param_u,param_d)

comp2_SC<-data.frame(matrix(unlist(comp2[[1]]), nrow=length(comp2[[1]]), byrow=TRUE)) %>%
  mutate(max_PageRank= X1, threshold= X2, Period= "Short-term after")%>%select(max_PageRank, threshold, Period)%>%
  group_by(threshold)


###Long-term after,
comp3<-getPR_of_dailyGraphs(edge_list=edgeList_longT_after, 
                            node_list=nodes_longT_after,
                            param_u,param_d)

comp3_SC<-data.frame(matrix(unlist(comp3[[1]]), nrow=length(comp3[[1]]), byrow=TRUE)) %>%
  mutate(max_PageRank= X1, threshold= X2, Period= "Long-term after")%>%select(max_PageRank, threshold, Period)%>%
  group_by(threshold)
...
#--------------------------------------------------------------------------------------------------#
###Plot average PageRank for a list of 365 networks

###Plots
p<-ggplot()+
  geom_step(data=comp1_SC, mapping=aes(x = threshold, y= max_PageRank, color= Period),
            direction = "vh",lwd = 1.2 )+
  geom_step(data=comp2_SC, mapping=aes(x = threshold, y= max_PageRank,color=Period),
            direction = "vh",lwd = 1.2   )+
  geom_step(data=comp3_SC, mapping=aes(x = threshold, y= max_PageRank,color=Period),
            direction = "vh",lwd = 1.2  )+
  xlab("Minimum days of LHC") +
  ylab("Maximum PageRank") +
  ...
  scale_y_continuous(expand = c(0, 0),limits = c(0, 0.014))+
  ...
  ggtitle(expression("Maximum PR score at daily LHC thresholds")) 

p

png(file = "maximum_PageRank_365thresholds.png", res = 300, width = 2000, height = 1315)
plot (p)
dev.off()
